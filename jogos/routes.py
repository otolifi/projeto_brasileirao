...
def create():
    ...
    escudos = list(Time.query.with_entities(Time.nome, Time.escudo))
    ...
    if form.validate_on_submit():
        ...
        try:
            ...
            return render_template('novojogo.html', title='Novo Jogo',
                                   form=form, times=times, escudos=escudos, jogo=None, errors=e)
        else:
    ...
    return render_template('novojogo.html', title='Novo Jogo', form=form,
                           jogo=None, times=times, escudos=escudos, errors=form.errors)
...
def update(id):
    ...
    if form.validate_on_submit():
        ...
        db.session.add(jogo)
        db.session.commit()
        flash('Jogo alterado com sucesso.')
        return redirect(url_for('jogos.index'))
    ...
    return render_template('novojogo.html', form=form, title="Editar Jogo",
                           jogo=jogo, escudos=escudos, errors=form.errors)
